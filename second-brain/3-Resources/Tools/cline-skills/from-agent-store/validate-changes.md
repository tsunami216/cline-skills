...
<!-- Description: After substantive code changes, validate that the change works as requested: map acceptance criteria, run targeted tests/smoke checks, fix failures, and report proven vs untested. Use when finishing an implementation or when the user asks to validate, verify, test, smoke-check, or confirm the Mac/app/feature works. -->
